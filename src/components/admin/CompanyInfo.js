...
let counter = 0;
function createData(balance, authID, phoneNumber, email, description) {
    counter += 1;

    //オブジェクトの分割代入？
    return { id: counter, balance, authID, phoneNumber, email, description };
...
class EnhancedTable extends React.Component {
    ...
    componentDidMount() {
        ...
        moon
            .get(`api/company/${this.state.id}`)
            .then(res =>{

                for(let i = 0; i < res.data.length; i++) {
                    this.setState({ data: this.state.data.concat([createData([res.data[i].paymentInfo.currentBalance],[res.data[i].authID],[res.data[i].phoneNumber],[res.data[i].email],[res.data[i].description] )])})
                }
            })
